...
let obj = {
  onImport: async function() {
    ...
    /* 
      should be: betterTTVEmotes = await goosemodScope.cspBypasser.json('https://api.betterttv.net/3/cached/emotes/global');
      ...
      for (let el of els) {
        ...
        if (betterTTVEnabled) for (let e of betterTTVEmotes) {
          ...
          for (let n of el.childNodes) {
            ...
            emoteReplace(n, el, e.code, await goosemodScope.cspBypasser.image(`https://cdn.betterttv.net/emote/${e.id}/2x.${e.imageType}`));
          }
        }
      }
    }, 500);
