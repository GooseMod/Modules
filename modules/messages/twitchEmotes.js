...
let obj = {
  onImport: async function() {
    ...
    /* 
      should be: betterTTVEmotes = await globalThis.cspBypasser.json('https://api.betterttv.net/3/cached/emotes/global');
      however slow to request for import time, so just use "cached" version
    */
  },
      ...
      for (let el of els) {
        ...
        if (betterTTVEnabled) for (let e of betterTTVEmotes) {
          ...
          for (let n of el.childNodes) {
            ...
            emoteReplace(n, el, e.code, await globalThis.cspBypasser.image(`https://cdn.betterttv.net/emote/${e.id}/2x.${e.imageType}`));
          }
        }
      }
    }, 500);

    globalThis.settings.createItem('Twitch Emotes', [
      `(v${version})`,
  ...
  remove: async function() {
    ...
    let settingItem = globalThis.settings.items.find((x) => x[1] === 'Twitch Emotes');
